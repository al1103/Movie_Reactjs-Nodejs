  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (data.status === "success") {
        ...
        // Navigation (with improved handling)
        const destination = data.data.role === "admin" ? "/admin" : "/";
        setTimeout(() => {
          // Use the history object for programmatic navigation if available
          if (Navigator && typeof Navigator === "function") {
            Navigator(destination);
          } else {
            window.location.href = destination;
          }
          window.location.reload(); // Reload after navigation
        }, 300); // 1-second delay for feedback
      } else {
        notify("Sai email hoặc mật khẩu");
      }
    } catch (error) {
      console.error("Login Error:", error); // More informative error logging
      notify("Đã xảy ra lỗi. Vui lòng thử lại."); // General error message for user
    } finally {
